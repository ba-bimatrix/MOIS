...
class FileOrganizationDAO(AbstractDAO, SingletonInstance):
    """
    ...
    def read(cls, session: AbstractSession, **params):
        ...
        ret = pd.read_csv(config.find("FileSource", "file.directory") + "/" + config.find("FileSource", "file.organization_data"), dtype=object)
