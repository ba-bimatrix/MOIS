...
class SampleDAO(AbstractDAO, SingletonInstance):
    """
    ...
    def list_route_resource(self, session: AbstractSession, **params):
        """
            ...
            """
                SELECT T1.*
                 ...
                 WHERE RESC_ID IN (
                                   SELECT RESC_ID
                                   FROM FS_BOR
                                   WHERE PROC_ID IN (select CURR_LOC_ID AS PROC_ID from FS_ROUTE where sim_id = :simulation_id and CURR_LOC_TYP = 'PROC')
                                      OR PROC_ID IN (select NEXT_LOC_ID AS PROC_ID from FS_ROUTE where sim_id = :simulation_id and NEXT_LOC_TYP = 'PROC')
                                  )
            """,
            params)

    def list_route_constraint(self, session: AbstractSession, **params):
        """
        세션 인스턴스를 통해 Data Source로부터 리스트 데이터를 조회
        :param session: AbstractSession 인스턴스
        :param params: sql 파라미터 데이터
    ...
    def list_route_constraint_max_priority(self, session: AbstractSession, **params):
        """
        세션 인스턴스를 통해 Data Source로부터 리스트 데이터를 조회
        :param session: AbstractSession 인스턴스
        :param params: sql 파라미터 데이터
